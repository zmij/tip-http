...
#include <pushkin/http/server/json_request_handler.hpp>

namespace psst {
namespace http {
namespace server {

LOCAL_LOGGING_FACILITY(JSONBODY, TRACE);
...
//----------------------------------------------------------------------------
void
json_reply::log_already_sent() const
{
    local_log(logger::WARNING) << "[#"
            << ::std::to_string(r_.serial()) << "] Reply already sent";
}
} /* namespace server */
} /* namespace http */
} /* namespace psst */
